...
async def get_sales_chart(
    period: str = "30d",  # 7d, 30d, 90d, 12m
    db: Session = Depends(get_db),
    current_user = Depends(get_current_user)
):
    """
    Retourne les données pour le graphique des ventes
    Period: 7d (7 jours), 30d (30 jours), 90d (90 jours), 12m (12 mois)
    """
    try:
        cache_key = _get_cache_key("sales_chart", period)
        ...
        def compute_chart_data():
            ...
            if period == "7d":
                ...
                # 90 derniers jours groupés par semaine
                ...
                for i in range(11, -1, -1):
                    ...
                    revenue = db.query(func.coalesce(func.sum(Invoice.total), 0)).filter(
                        func.extract('month', Invoice.date) == month_date.month,
                        func.extract('year', Invoice.date) == month_date.year,
                        Invoice.status.in_(["payée", "PAID"])
                    ).scalar() or 0
        ...
        result = _get_cached_or_compute(cache_key, compute_chart_data)
        return result
        
    except Exception as e:
        logging.error(f"Erreur sales chart: {e}")
        return {"labels": [], "data": [], "period": period, "error": str(e)}
