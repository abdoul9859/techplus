...
async def list_quotations_paginated(
    ...
    start_date: Optional[DateType] = None,
    end_date: Optional[DateType] = None,
    sort_by: Optional[str] = Query("created_at"),  # created_at | date | number | total | status | sent
    ...
    if key == 'number':
        ...
        order = Quotation.created_at.desc() if desc_dir else Quotation.created_at.asc()
    else:
